...
##
# Executes a command without having to manually check it
# Usage: ./execute.sh "./manage.py startapp nba"
##
...
function remove_old_migrations() {
	if rm -rf "$1"/migrations/00* ; then
		success "Reset: Migrations from $1 are now removed"
	else
		fail "Reset: Migrations from $1 failed to be removed"
	fi
}

# Reads in secret values
info "Reset: Reading in secret values"
. ../db/secret_settings.sh

# Drops the whole db
if [[ $(sudo -u postgres psql postgres -tAc "\l" | grep "$db_name") != "" ]]; then
	info "Reset: Database detected, dropping now"
	if sudo -u postgres psql postgres -tAc "DROP DATABASE \"$db_name\";" ; then
		success "Reset: Database $db_name is now dropped"
	else
		fail "Reset: Database $db_name failed to be dropped"
	fi
else
	success "Reset: Database $db_name is already dropped"
fi

# Removes all prior migrations
#	remove_old_migrations "core"
